# 👋 Hi, I'm David Ambros

## Entry-Level Cybersecurity Professional | Google Certified

Transitioning into cybersecurity from healthcare administration, where I identified **12+ phishing attempts** protecting 1,000+ patient records. Combining real-world threat detection experience with technical security training.

### 🎯 Current Focus
Building SOC Analyst capabilities through hands-on learning and professional certifications. Seeking **SOC Analyst** or **Junior Security Analyst** roles in the UK.

---

## 🛠️ Technical Skills

### Security Fundamentals
![Python](https://img.shields.io/badge/-Python-3776AB?style=flat&logo=python&logoColor=white)
![Linux](https://img.shields.io/badge/-Linux-FCC624?style=flat&logo=linux&logoColor=black)
![Wireshark](https://img.shields.io/badge/-Wireshark-1679A7?style=flat&logo=wireshark&logoColor=white)
![Splunk](https://img.shields.io/badge/-Splunk-000000?style=flat&logo=splunk&logoColor=white)

**Security Knowledge:**  
SIEM Tools • Python Scripting • Linux Security • Network Analysis • Log Analysis • Incident Response • Threat Detection • GDPR Compliance

**Frameworks:**  
NIST Cybersecurity Framework • ISO 27001 Principles

---

## 📜 Certifications

✅ **Google Cybersecurity Professional Certificate** | Coursera | 2025  
*8-course program covering: SIEM tools, Python, Linux, SQL, Network Security, Incident Response, Threat Detection, Vulnerability Assessment*

✅ **Cisco Cybersecurity Academy** | 2024  
*Network security fundamentals and cybersecurity principles*

🎯 **CompTIA Security+** | Currently Studying

---

## 💼 Real-World Security Experience

### Threat Detection in Healthcare Operations

**What I Did:**
- 🎯 Identified and reported **12+ phishing attempts** targeting patient data over 12 months
- 🛡️ Protected **1,000+ confidential patient records** from potential GDPR violations
- 📊 Managed sensitive patient data in compliance with GDPR and healthcare regulations
- 🔍 Analyzed clinical correspondence daily for security threats and anomalies
- 👥 Collaborated with IT security team on incident reporting and response procedures

**Why This Matters:**  
Unlike most entry-level candidates, I have hands-on experience identifying actual threats in a live-healthcare environment. I understand:
- How phishing attacks target healthcare organizations
- The business impact of security incidents
- GDPR compliance requirements in regulated environments
- How to communicate security risks to non-technical staff

---

## 📈 Active Learning

**Daily Practice:**
- **TryHackMe** - Hands-on security challenges and CTF exercises
- **CompTIA Security+ Study** - Preparing for industry-standard certification
- **Python & Linux** - Building scripting and system administration capabilities

**Completed Training:**
- Google Cybersecurity Professional Certificate (comprehensive 8-course program)
- Cisco Cybersecurity Academy fundamentals
- SIEM analysis and log monitoring techniques
- Incident response procedures and frameworks
- Network security and packet analysis with Wireshark

---

## 💡 What Makes Me Different

My healthcare background provides unique advantages:

1. **Real Threat Detection** - Identified actual phishing attempts in live operations, not just training scenarios
2. **Regulatory Compliance** - Deep understanding of GDPR, patient data protection, and healthcare security requirements
3. **Non-Technical Communication** - Can explain complex security concepts to staff without technical backgrounds
4. **High-Stakes Environment** - Experience managing sensitive data where breaches have serious consequences

**This makes me especially valuable for:**
- Healthcare organizations
- Financial services
- Any regulated industry requiring security + compliance knowledge

---

## 🌐 Connect With Me

[![LinkedIn](https://img.shields.io/badge/-LinkedIn-0077B5?style=flat&logo=linkedin&logoColor=white)].
[![Email](https://img.shields.io/badge/-Email-D14836?style=flat&logo=gmail&logoColor=white)](mailto:your-email@email.com)

📍 **Location:** United Kingdom  
💼 **Open to:** SOC Analyst | Security Analyst | CyberSecurity Analyst | Junior Security Operations roles   
🔎 **Status:** Actively seeking entry-level cybersecurity opportunities

---

*"From protecting patient data to protecting all data - bringing real-world security experience to my first cybersecurity role."*
